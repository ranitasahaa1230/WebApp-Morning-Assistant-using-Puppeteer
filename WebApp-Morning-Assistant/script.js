...
(async function () {
    ...
    await tab.goto(waLink, {waitUntil: "networkidle2"}, { delay: 50 });

    await tab.waitForSelector("._2_1wd.copyable-text.selectable-text");
    await tab.type("._2_1wd.copyable-text.selectable-text", "Daddy", { delay: 50 });
...
async function getNumbericReply(tab, browser) {
    ...
    if (reply.localeCompare("1") == 0) {
        ...
        let innertext = "   Misty with Light Winds. ";
        ...
        await tab.waitForSelector("#main div.selectable-text[contenteditable]", { visible: true });
        await tab.type("#main div.selectable-text[contenteditable]", 'Today  is know as -'+ text  + today);
        await tab.keyboard.press("Enter");
